...
AugCoeffMatrix <-function(mainList){
  ...
  unknowns = unknowns + 1 #increment for x0 gap
  ...
  mainData = list(variables = mainVariables, augcoeffmatrix = matrix(nrow = numOfEq, ncol = numOfEq+1,dimnames = list(c(1:numOfEq),c(mainVariables,"RHS"))))
  print(mainData)
