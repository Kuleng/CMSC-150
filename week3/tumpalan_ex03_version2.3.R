...
E3 <- function(x0,x1,x2) 7*x1+-0.3*x2+19.3;
...
AugCoeffMatrix <-function(mainList){
  ...
  for (a in mainList){
    ...
    while ( loopCount < length(splittedCoeffs) - 1){
      loopCount = loopCount + 1
      if(loopCount == (numOfEq+1)){
        mainData$augcoeffmatrix[outerLoopCount,loopCount] = (-1)*as.numeric(substring(splittedCoeffs[[loopCount]][1],1))
      }     #given x^n of the function element, if n == length-1 of matrix index (means that it has correct position)
      else if(is.na(as.numeric(substring(splittedCoeffs[[loopCount]][2],2))) || as.numeric(substring(splittedCoeffs[[loopCount]][2],2)) == loopCount-1){ #while x^n, if n == col.
    ...
    while ( loopCount < numOfEq+1){
      ...
      else if (loopCount <= length(splittedCoeffs)){#problematic
